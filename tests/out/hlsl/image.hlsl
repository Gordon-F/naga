Texture2D<uint4> image_src : register(t1);
RWTexture1D<uint4> image_dst : register(u2);
Texture1D<float4> image_1d : register(t0);
Texture2D<float4> image_2d : register(t1);
Texture2DArray<float4> image_2d_array : register(t2);
TextureCube<float4> image_cube : register(t3);
TextureCubeArray<float4> image_cube_array : register(t4);
Texture3D<float4> image_3d : register(t5);
Texture2DMS<float4> image_aa : register(t6);
SamplerState sampler_reg : register(s0);
SamplerComparisonState sampler_cmp : register(s1);
Texture2D image_2d_depth : register(t2);

struct ComputeInput_main {
    uint3 local_id1 : SV_GroupThreadID;
};

int2 Naga2DDimensions(Texture2D<uint4>)
{
    uint4 ret;
    image_src.GetDimensions(0, ret.x, ret.y, ret.z);
    return ret.xy;
}

[numthreads(16, 1, 1)]
void main(ComputeInput_main computeinput_main)
{
    int2 dim = Naga2DDimensions(image_src);
    int2 itc = (mul(dim, int2(computeinput_main.local_id1.xy)) % int2(10, 20));
    uint4 value = image_src.Load(int3(itc, 0));
    image_dst[itc.x] = value;
    return;
}

int Naga1DDimensions(Texture1D<float4>)
{
    uint4 ret;
    image_1d.GetDimensions(ret.x);
    return ret.x;
}

int2 Naga2DDimensions(Texture2D<float4>)
{
    uint4 ret;
    image_2d.GetDimensions(0, ret.x, ret.y, ret.z);
    return ret.xy;
}

int Naga2DNumLevels(Texture2D<float4>)
{
    uint4 ret;
    image_2d.GetDimensions(0, ret.x, ret.y, ret.z);
    return ret.z;
}

int2 Naga2DMipDimensions(Texture2D<float4>, uint MipLevel)
{
    uint4 ret;
    image_2d.GetDimensions(MipLevel, ret.x, ret.y, ret.z);
    return ret.xy;
}

int2 Naga2DArrayDimensions(Texture2DArray<float4>)
{
    uint4 ret;
    image_2d_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.xy;
}

int Naga2DArrayNumLevels(Texture2DArray<float4>)
{
    uint4 ret;
    image_2d_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.w;
}

int2 Naga2DArrayMipDimensions(Texture2DArray<float4>, uint MipLevel)
{
    uint4 ret;
    image_2d_array.GetDimensions(MipLevel, ret.x, ret.y, ret.z, ret.w);
    return ret.xy;
}

int Naga2DArrayNumLayers(Texture2DArray<float4>)
{
    uint4 ret;
    image_2d_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.w;
}

int2 NagaCubeDimensions(TextureCube<float4>)
{
    uint4 ret;
    image_cube.GetDimensions(0, ret.x, ret.y, ret.z);
    return ret.xy;
}

int NagaCubeNumLevels(TextureCube<float4>)
{
    uint4 ret;
    image_cube.GetDimensions(0, ret.x, ret.y, ret.z);
    return ret.z;
}

int2 NagaCubeMipDimensions(TextureCube<float4>, uint MipLevel)
{
    uint4 ret;
    image_cube.GetDimensions(MipLevel, ret.x, ret.y, ret.z);
    return ret.xy;
}

int2 NagaCubeArrayDimensions(TextureCubeArray<float4>)
{
    uint4 ret;
    image_cube_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.xy;
}

int NagaCubeArrayNumLevels(TextureCubeArray<float4>)
{
    uint4 ret;
    image_cube_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.w;
}

int2 NagaCubeArrayMipDimensions(TextureCubeArray<float4>, uint MipLevel)
{
    uint4 ret;
    image_cube_array.GetDimensions(MipLevel, ret.x, ret.y, ret.z, ret.w);
    return ret.xy;
}

int NagaCubeArrayNumLayers(TextureCubeArray<float4>)
{
    uint4 ret;
    image_cube_array.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.w;
}

int3 Naga3DDimensions(Texture3D<float4>)
{
    uint4 ret;
    image_3d.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.xyz;
}

int Naga3DNumLevels(Texture3D<float4>)
{
    uint4 ret;
    image_3d.GetDimensions(0, ret.x, ret.y, ret.z, ret.w);
    return ret.w;
}

int3 Naga3DMipDimensions(Texture3D<float4>, uint MipLevel)
{
    uint4 ret;
    image_3d.GetDimensions(MipLevel, ret.x, ret.y, ret.z, ret.w);
    return ret.xyz;
}

int Naga2DMSNumSamples(Texture2DMS<float4>)
{
    uint4 ret;
    image_aa.GetDimensions(ret.x, ret.y, ret.z);
    return ret.z;
}

float4 queries() : SV_Position
{
    int dim_1d = Naga1DDimensions(image_1d);
    int2 dim_2d = Naga2DDimensions(image_2d);
    int num_levels_2d = Naga2DNumLevels(image_2d);
    int2 dim_2d_lod = Naga2DMipDimensions(image_2d, 1);
    int2 dim_2d_array = Naga2DArrayDimensions(image_2d_array);
    int num_levels_2d_array = Naga2DArrayNumLevels(image_2d_array);
    int2 dim_2d_array_lod = Naga2DArrayMipDimensions(image_2d_array, 1);
    int num_layers_2d = Naga2DArrayNumLayers(image_2d_array);
    int2 dim_cube = NagaCubeDimensions(image_cube);
    int num_levels_cube = NagaCubeNumLevels(image_cube);
    int2 dim_cube_lod = NagaCubeMipDimensions(image_cube, 1);
    int2 dim_cube_array = NagaCubeArrayDimensions(image_cube_array);
    int num_levels_cube_array = NagaCubeArrayNumLevels(image_cube_array);
    int2 dim_cube_array_lod = NagaCubeArrayMipDimensions(image_cube_array, 1);
    int num_layers_cube = NagaCubeArrayNumLayers(image_cube_array);
    int3 dim_3d = Naga3DDimensions(image_3d);
    int num_levels_3d = Naga3DNumLevels(image_3d);
    int3 dim_3d_lod = Naga3DMipDimensions(image_3d, 1);
    int num_samples_aa = Naga2DMSNumSamples(image_aa);
    int sum = ((((((((((((((((((dim_1d + dim_2d.y) + dim_2d_lod.y) + dim_2d_array.y) + dim_2d_array_lod.y) + num_layers_2d) + dim_cube.y) + dim_cube_lod.y) + dim_cube_array.y) + dim_cube_array_lod.y) + num_layers_cube) + dim_3d.z) + dim_3d_lod.z) + num_samples_aa) + num_levels_2d) + num_levels_2d_array) + num_levels_3d) + num_levels_cube) + num_levels_cube_array);
    return float4(float(sum).xxxx);
}

float4 sample1() : SV_Target0
{
    float2 tc = float2(0.5.xx);
    float4 s2d = image_2d.Sample(sampler_reg, tc);
    float4 s2d_offset = image_2d.Sample(sampler_reg, tc, int2(3, 1));
    float4 s2d_level = image_2d.SampleLevel(sampler_reg, tc, 2.3);
    float4 s2d_level_offset = image_2d.SampleLevel(sampler_reg, tc, 2.3, int2(3, 1));
    return (((s2d + s2d_offset) + s2d_level) + s2d_level_offset);
}

float sample_comparison() : SV_Target0
{
    float2 tc = float2(0.5.xx);
    float s2d_depth = image_2d_depth.SampleCmp(sampler_cmp, tc, 0.5);
    float s2d_depth_level = image_2d_depth.SampleCmpLevelZero(sampler_cmp, tc, 0.5);
    return (s2d_depth + s2d_depth_level);
}
